export default {
  translation: {
    ...
    login: {
      ...
      continue: 'Continue',
      title: 'Welcome to Converged Retrieval Knowledge Base Platform',
      start: "Let's get started",
      description:
        'Base Converage Database, explore top RAG technology. Create knowledge bases and AIs to empower your business.',
      review: 'Welcome reviews',
    },
    header: {
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
        'Conduct a retrieval test to check if ConvergedRAG can recover the intended content for the LLM.',
      Parse: 'Parse',
      dataset: 'Dataset',
      ...
      rerankTip: `Optional. If left empty, ConvergedRAG will use a combination of weighted keyword similarity and weighted vector cosine similarity; if a rerank model is selected, a weighted reranking score will replace the weighted vector cosine similarity. Please be aware that using a rerank model will significantly increase the system's response time. If you wish to use a rerank model, ensure you use a SaaS reranker; if you prefer a locally deployed rerank model, ensure you start ConvergedRAG with docker-compose-gpu.yml.`,
      topK: 'Top-K',
...
<ul>
  ...
  <li>The auto-keyword feature is dependent on the LLM and consumes a significant number of tokens.</li>
    ...
    chat: {
      ...
      betaError:
        'Please acquire a ConvergedRAG API key from the System Settings page first.',
      searching: 'Searching...',
      parsing: 'Parsing',
      uploading: 'Uploading',
      ...
      removeKnowledgePlaceholder: 'Please remove {knowledge} from system prompt since no knowledge base or Tavily is used',
...
Example: https://fsn1.your-objectstorage.com`,
      ...
      embeddingModelTip:
        'The default embedding model for each newly created knowledge base. If you cannot find an embedding model from the dropdown, check if you are using ConvergedRAG slim edition (which does not include embedding models) or check https://ragflow.io/docs/dev/supported_models to see if your model provider supports this model.',
      img2txtModel: 'VLM',
      img2txtModelTip:
