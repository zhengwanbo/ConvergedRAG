...
class OracleDatabase(Database):
    ...
    def init(self, database, **kwargs):
        params = {
            'encoding': 'UTF-8',  # Oracle uses encoding instead of charset
            'nencoding': 'UTF-8',  # National character set encoding
            'threaded': True,  # Recommended for Oracle connections
        }
        params.update(kwargs)

        # Handle Oracle-specific parameter conversions
        if 'password' in params:
            params['password'] = params['password']  # No special handling needed for Oracle

        # Oracle uses service_name/sid in dsn rather than database name
        if 'host' in params and 'port' in params:
            service_name = database  # Using database name as service name
            params['dsn'] = f"{params['host']}:{params['port']}/{service_name}"
            del params['host']
            del params['port']

        super(OracleDatabase, self).init(database, **params)
