...
const Login = () => {
  ...
  const changeTitle = () => {
    // Disable switching to register mode completely
    if (title === 'login') {
      return; // Don't allow switching to register
    }
    
    setIsLoginPage(title !== 'login');
    setTimeout(() => {
      setTitle('login'); // Always switch back to login
    }, 200);
  };
  ...
  const onCheck = async (params: { email: string; password: string; remember?: boolean }) => {
    ...
    try {
      ...
      // Only handle login, no registration
      ...
      <div className=" h-[inherit] relative overflow-auto">
        ...
        <div className="absolute top-3 flex flex-col items-center mb-12 w-full text-text-primary">
          <div className="flex items-center mb-4 w-full pl-10 pt-10 ">
            ...
            <div className="text-xl font-bold self-center">ConvergedRAG</div>
          </div>
