...
export const similarityThresholdSchema = { 
  similarity_threshold: z.coerce
    .number()
    .min(0)
    .max(1)
    .transform((val) => Math.round(val * 100) / 100), // 四舍五入到小数点后两位
};

export const keywordsSimilarityWeightSchema = {
  keywords_similarity_weight: z.coerce
    .number()
    .min(0)
    .max(1)
    .transform((val) => Math.round(val * 100) / 100), // 四舍五入到小数点后两位
...
export const vectorSimilarityWeightSchema = {
  vector_similarity_weight: z.coerce
    .number()
    .min(0)
    .max(1)
    .transform((val) => Math.round(val * 100) / 100), // 四舍五入到小数点后两位
            ...
            <div className={cn('flex items-end gap-14 justify-between')}>
              ...
              <FormControl>
                <NumberInput
                  ...
                  step={0.01}
                  {...field}
                  value={field.value ? parseFloat(field.value.toFixed(2)) : 0}
                ></NumberInput>
              </FormControl>
            </div>
            <FormMessage />
