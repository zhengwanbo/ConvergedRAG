export default {
  translation: {
    ...
    login: {
      ...
      start: '立即开始',
      description:
        '集成融合数据库，利用融合检索 RAG 技术，创建知识库和人工智能来增强您的业务',
      review: '欢迎评论',
    },
    header: {
      knowledgeBase: '知识库',
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
        '进行检索测试，检查 ConvergedRAG 是否能够为大语言模型（LLM）恢复预期的内容。',
      Parse: '解析',
      dataset: '知识库',
      testing: '检索测试',
      configuration: '配置',
      ...
      rerankTip: `非必选项：若不选择 rerank 模型，系统将默认采用关键词相似度与向量余弦相似度相结合的混合查询方式；如果设置了 rerank 模型，则混合查询中的向量相似度部分将被 rerank 打分替代。请注意：采用 rerank 模型会非常耗时。如需选用 rerank 模型，建议使用 SaaS 的 rerank 模型服务；如果你倾向使用本地部署的 rerank 模型，请务必确保你使用 docker-compose-gpu.yml 启动 ConvergedRAG。`,
      topK: 'Top-K',
...
`,
    ...
    <p>
      ...
      您只需与<i>'ConvergedRAG'</i>交谈即可列出所有符合资格的候选人。
      </p>
        `,
...
General：实体和关系提取提示来自 GitHub - microsoft/graphrag：基于图的模块化检索增强生成 (RAG) 系统`,
    ...
    chat: {
      ...
      removeKnowledgePlaceholder: '请从系统提示中移除 {knowledge}，因为没有使用知识库或Tavily',
      crossLanguage: '跨语言搜索',
...
示例：general/v2/`,
      ...
      embeddingModelTip:
        '所有新创建的知识库使用的默认嵌入模型。如未显示可选模型，请检查你是否在使用 ConvergedRAG slim 版(不含嵌入模型)；或根据 https://ragflow.io/docs/dev/supported_models 确认你的模型供应商是否提供该模型。',
      img2txtModel: 'VLM',
      img2txtModelTip:
      ...
      modelProvidersWarn: `请先在<b>大模型管理</b>中添加嵌入模型和LLM，然后在“设置默认模型”中设置它们。`,
      apiVersion: 'API版本',
...
[在此处插入文本]`,
    ...
    admin: {
      ...
      passwordMinLength: '密码长度必须大于8个字符',
      ...
      importWhitelist: '导入白名单（Excel格式）',
      importSelectExcelFile: 'Excel文件（.xlsx格式）',
      importOverwriteExistingEmails: '覆盖现有邮箱',
      importInvalidExcelFile: '请选择有效的Excel文件',
      importFileRequired: '请选择待导入的文件',
      importFileTips:
        '文件需包含唯一表头列，列名必须为 <code>email</code>',
