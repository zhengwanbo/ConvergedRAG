  ...
  translation: {
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
        'Проведите тест извлечения, чтобы проверить, может ли Agent Factory  восстановить целевой контент для LLM.',
      Parse: 'Парсинг',
      dataset: 'Датасет',
      ...
      rerankTip: `Опционально. Если оставить пустым, Agent Factory  будет использовать комбинацию взвешенной схожести по ключевым словам и взвешенного косинусного сходства векторов; если выбрана rerank модель, взвешенный reranking счет заменит взвешенное косинусное сходство векторов. Имейте в виду, что использование rerank модели значительно увеличит время отклика системы. Если вы хотите использовать rerank модель, убедитесь, что вы используете SaaS reranker; если вы предпочитаете локально развернутую rerank модель, убедитесь, что вы запускаете Agent Factory  с помощью docker-compose-gpu.yml.`,
      topK: 'Топ-K',
...
<ul>
  ...
  <li>Функция авто-ключевых слов зависит от LLM и потребляет значительное количество токенов.</li>
    ...
    chat: {
      ...
      betaError:
        'Пожалуйста, сначала получите Agent Factory  API ключ на странице Системных настроек.',
      searching: 'Поиск...',
      parsing: 'Парсинг',
      uploading: 'Загрузка',
...
Пример: Virtual Hosted Style`,
      ...
      embeddingModelTip:
        'Модель эмбеддингов по умолчанию для каждой новой созданной базы знаний. Если вы не можете найти модель эмбеддингов в выпадающем списке, проверьте, используете ли вы облегченную версию Agent Factory  (которая не включает модели эмбеддингов) или проверьте https://ragflow.io/docs/dev/supported_models, чтобы узнать, поддерживает ли ваш провайдер моделей эту модель.',
      img2txtModel: 'VLM',
      img2txtModelTip:
