export default {
  translation: {
    ...
    login: {
      ...
      register: '创建账户',
      continue: '继续',
      title: '智能 AI Agent 工厂, 构建您私有 Agent.',
      start: '立即开始',
      description:
        '利用RAG与Agent技术, 创建知识库和人工智能Agent来增强您的业务',
      review: '来自 500 多条评论',
    },
    header: {
      knowledgeBase: '知识库',
...
例如：一条带有 1024 维嵌入的 1 KB 消息大约使用 9 KB。5 MB 的默认限制大约可容纳 500 条此类消息。`,
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
        '进行检索测试，检查 Agent Factory  是否能够为大语言模型（LLM）恢复预期的内容。',
      Parse: '解析',
      dataset: '知识库',
      testing: '检索测试',
      configuration: '配置',
      ...
      testingDescription:
        '请完成召回测试：确保你的配置可以从数据库召回正确的文本块。如果你调整了这里的默认设置，比如关键词相似度权重，请注意这里的改动不会被自动保存。请务必在问答助手设置或者召回算子设置处同步更新相关设置。',
      ...
      rerankTip: `非必选项：若不选择 rerank 模型，系统将默认采用关键词相似度与向量余弦相似度相结合的混合查询方式；如果设置了 rerank 模型，则混合查询中的向量相似度部分将被 rerank 打分替代。请注意：采用 rerank 模型会非常耗时。如需选用 rerank 模型，建议使用 SaaS 的 rerank 模型服务；如果你倾向使用本地部署的 rerank 模型，请务必确保你使用 docker-compose-gpu.yml 启动 Agent Factory 。`,
      topK: 'Top-K',
...
`,
    ...
    <p>
      ...
      您只需与<i>'Agent Factory '</i>交谈即可列出所有符合资格的候选人。
      </p>
        `,
...
General：实体和关系提取提示来自 GitHub - microsoft/graphrag：基于图的模块化检索增强生成 (RAG) 系统`,
    ...
    chat: {
      ...
      chatConfiguration: '问答配置',
      chatConfigurationDescription: '为你的知识库配置专属问答助手！ 💕',
      assistantName: '助理姓名',
      ...
      system: '系统提示词',
      systemInitialValue: `你是一个智能助手，请总结知识库的内容来回答问题，请列举知识库中的数据详细回答。当所有知识库内容都与问题无关时，你的回答必须包括“知识库中未找到您要的答案！”这句话。回答需要考虑问答历史。
        以下是知识库：
        {knowledge}
        以上是知识库。`,
      systemMessage: '请输入',
...
示例：general/v2/`,
      ...
      discordDescription: ' 连接你的 Discord 服务器以访问和分析问答数据。',
      ...
      userManagement: '用户管理',
      manageUsers: '管理用户',
      manageUsersForbidden: '只有管理员可以访问用户管理。',
      team: '团队管理',
      system: '系统',
      logout: '登出',
      api: 'API',
      ...
      chatModel: 'LLM',
      chatModelTip: '所有新创建的知识库都会使用默认的大语言模型。',
      ttsModel: 'TTS',
      ttsModelTip:
        '默认的tts模型会被用于在对话过程中请求语音生成时使用。如未显示可选模型，请根据 https://ragflow.io/docs/dev/supported_models 确认你的模型供应商是否提供该模型。',
      embeddingModel: 'Embedding',
      embeddingModelTip:
        '所有新创建的知识库使用的默认嵌入模型。如未显示可选模型，请检查你是否在使用 Agent Factory slim 版(不含嵌入模型)；或根据 https://ragflow.io/docs/dev/supported_models 确认你的模型供应商是否提供该模型。',
      img2txtModel: 'VLM',
      img2txtModelTip:
